...
<template>
      <form @submit.prevent="handleLogin" class="flex flex-col gap-y-4 sm:gap-y-6">
        <div v-if="error" class="rounded-lg bg-red-100 p-3 text-sm text-red-700 dark:bg-red-900/20 dark:text-red-400">
          {{ error }}
        </div>
        <AppFormField v-model="email" type="email" id="email" required>Email</AppFormField>
        <AppFormField v-model="password" type="password" id="password" required>Password</AppFormField>
